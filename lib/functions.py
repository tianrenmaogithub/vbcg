...
# retrieve vbcg genes
def parse_hmm_out_original(hmm_out):
    '''
    output: hmm ID - protein ID
    '''
...
def parse_hmm_out(hmm_out):
    ...
    hmm = open(hmm_out, 'r').readlines()
    hmm = [i[:100] for i in hmm]
    hmm = [i for i in hmm if not i.startswith('#')]

    return(dict([i.split()[1:3] for i in hmm]))
...
def concatenate_genes_original(infiles, outdir):
    dict1 = {}
    for i in infiles:
...
def concatenate_genes(infiles, outdir, missing_genes=4):
    dict1 = {}
    for i in infiles:
        dict1[i] = SeqIO.to_dict(SeqIO.parse(i, 'fasta'))

    df = pd.DataFrame.from_dict(dict1)
    #df = df.loc[df.isnull().sum(axis=1)<=missing_genes,:]
    df = df.loc[df.notnull().sum(axis=1) >= 20 - missing_genes,:]
    df.dropna(axis=1, how='all', inplace=True)

    def replace_nan_with_placeholder(series):
        # Find the first non-nan element in the series
        first_non_nan = series.dropna().iloc[0] if not series.dropna().empty else None
        if first_non_nan is None:  # All elements are nan, return the original series
            return series

        placeholder_len = len(first_non_nan)
        placeholder_seq = Seq("?" * placeholder_len)  # Create a sequence of '?' of the same length as the first non-nan element
        placeholder_record = SeqRecord(placeholder_seq, id='NA', description='Placeholder')

        # Replace nan elements with the placeholder sequence record
        return series.apply(lambda x: placeholder_record if pd.isna(x) else x)

    # Apply the function to each column
    df_transformed = df.apply(replace_nan_with_placeholder)

    recs = []
    for i in df.index:
        rec = SeqRecord(id=i, seq=Seq(''.join([str(j.seq) for j in df_transformed.loc[i,:]])))
        recs.append(rec)
...
def make_tree(dir1, tree_maker, cpu, dir2, missing_genes=4):
    ...
    con = concatenate_genes(glob('*-gb'), '.', missing_genes)
